...
public class DTNHost implements Comparable<DTNHost>, Iterable<Connection> {
	...
	public DTNHost(List<MessageListener> msgLs,
		...
		this.ACKlist = new HashMap<String, Acknowledge>();
		...
		if(temp*temp==nrOfCells){
			...
			throw new SimError("Parameter nrofCells (" + 
								nrOfCells + ") is not a perfect square");
		}
		double x,y;
		int cnt = 0;
		for(int i=0;i<Math.sqrt(nrOfCells);i++)
		{
			 x=(i)*(maxX/(Math.sqrt(nrOfCells)))+((maxX/(Math.sqrt(nrOfCells)))/2);
			for(int j = 0;j<Math.sqrt(nrOfCells);j++)
			{
				y = (j)*(maxY/(Math.sqrt(nrOfCells)))+((maxY/(Math.sqrt(nrOfCells)))/2);
				arr[cnt++]=new Coord(x,y);		
			}
		}
	...
	public 	Coord getCentralLocation()
	{

		//Coord pnt = findPoint() ;  
		int index=0;	
		double dist=arr[0].distance(this.location);
		for(int i=1;i<nrOfCells;i++){
				if(dist<arr[i].distance(this.location)){
						index = i;
						dist =arr[i].distance(this.location); 
					}				
		}
	...
	public boolean hasMessage(String id) {
		return this.router.hasMessage(id);
	}
	...
	public HashMap<String, Acknowledge> getACKList( ){
		return ACKlist;
		
	}
	public  void put_ACK(Acknowledge Ack){
                /*create list*/
		ACKlist.put(Ack.getackString(), Ack);
	}
        public void showACKList(){
    
        
        for(Acknowledge ack : ACKlist.values())
          System.out.print(ack.getackString()+" ");
	System.out.println("-----------------------");
      
        }
        public boolean checkAcklist(String ack)
        {
           
             if(this.ACKlist.containsKey(ack)){
				return false;
			}	
	     return true;
