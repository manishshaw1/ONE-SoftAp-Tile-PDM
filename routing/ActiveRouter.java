...
public abstract class ActiveRouter extends MessageRouter {
	...
	/**
        * modified
        */
       private boolean check_duplicacy(Message m, Connection con){
                DTNHost peer = con.getOtherNode(getHost());
		if (peer.hasMessage(m.getId()) || !peer.checkAcklist(m.getId())) {
		         return true;
			}
			return true;
	...
	protected int startTransfer(Message m, Connection con) {
		...
		if(check_duplicacy(m,con)==true)
		{

          //   System.out.println(m.getId()+" true no duplicacy");
                if (!con.isReadyForTransfer()) {
    					return TRY_LATER_BUSY;
					}
		...
		for (int i=0; i<this.sendingConnections.size(); ) {
			...
			if(con.isTransferring()) {
				...
				if(!con.isSender(to))
				{
			        ...
			        intersect1.removeAll(set1);
			        set1.addAll(intersect1);
			        to.setSet(set1);
			        from.setSet(set2);
...
//MY CODE END
	...
	public boolean CanAcknowledge(Connection con,Message m){
	//return deliveredMessages.isEmpty();
	//checking whether the reciever node is the final recipent of the message or not
	if(con.getOtherNode(getHost()).toString().equals(m.getTo().toString()))
		 return true;

	else
		return false;
}
public void ackGeneration(Connection con, Message msg)
    {
       if(CanAcknowledge(con,msg))
       {
			//if recipent node is final recipent dd ack to host ack list.  no need of any useless iteration..
			this.getHost().put_ACK(new Acknowledge(msg.getId(),getHost()));


		/*	Set set = this.getHost().getACKList().entrySet();
  			Iterator x = set.iterator();
	 		while(x.hasNext())
	 		{
	 		Map.Entry me = (Map.Entry)x.next();
            Acknowledge ack = new Acknowledge((String)me.getKey(),getHost());

	  			if(con.isUp()){
              			if(con.getOtherNode(getHost()).checkAcklist(msg.getId()))
                			 {
                			 	con.getOtherNode(getHost()).put_ACK(ack);
                			 }
          // System.out.println(con.getOtherNode(getHost())+":");
           //con.getOtherNode(getHost()).showACKList();
	   			}
         	}*/
        }
    }
 public void sendAckList(Connection con,Message msg)
    {
   	DTNHost x = getHost();
	DTNHost y = con.getOtherNode(getHost());
        HashMap<String, Acknowledge> temp= new HashMap<String, Acknowledge> ();
        temp=x.getACKList();
        ...
        /* list transfer*/
       if(!y.checkAcklist((String)me.getKey()))
        {
  ...
  public void deleteAckMessages(){
   ...
   ArrayList<Message> temp =
        ...
        for (Message m : temp)  {
        		//System.out.println(m.getId() + host.showACKList());
				if (!host.checkAcklist(m.getId())) {
						removeDeliveredMessages(m.getId());
					//this.deleteMessage(m.getId(), false);
				}
